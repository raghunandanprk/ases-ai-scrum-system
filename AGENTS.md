...
## Context Integration
Obey all `<ases-state>` blocks injected by `ases-guard.ts`. They are live project state.
<!-- END ASES-MANAGED -->
...
/.kilo/          skills(36), agents(4), plugin, rules, system.yaml
...
7. Git commit only after UAT — `ases-guard.ts` enforces
